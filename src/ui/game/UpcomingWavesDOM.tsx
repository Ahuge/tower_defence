      ...
      {waves.length === 0 ? (
        ...
        waves.map((w, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontFamily: "'VT323', ui-monospace, monospace", fontSize: '12px', color: i === 0 ? 'var(--gold)' : 'var(--text-dim)', minWidth: '42px' }}>
              {w.label}
            </span>
            <span style={{ fontSize: '10px', color: 'var(--text-primary)', flex: 1 }}>
              {w.creepTypes}
            </span>
            <span style={{ fontFamily: "'VT323', ui-monospace, monospace", fontSize: '11px', color: 'var(--text-muted)' }}>x{w.count}</span>
            {w.isBoss && (
