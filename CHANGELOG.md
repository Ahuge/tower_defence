...
Earlier today items 1-4 shipped (`MissionRunner.getActiveLifecycle`, `UISurfaceAspect.endingPanel`, ADR realignment, data-passthrough freeze). Items 5-7 close the remaining audit gaps so the next campaign can be authored without re-litigating any of these architectural decisions.

- **Item 5 — Drop optional chaining from `WorldMutator` host dispatch.** `WorldHost` previously declared every install/remove method optional (`installPrePlacedTowers?(towers)`) and `WorldMutatorImpl` forwarded via `this.host.installPrePlacedTowers?.(towers)`. That meant a missing GameScene host method silently no-op'd at runtime — exactly the class of bug that produced the Phase E3 Arcane pre-placed Frost regression caught earlier today. Item 5 makes every `WorldHost` method REQUIRED; deleting a host method without removing its aspect caller no longer compiles. GameScene implements the latent narrow primitives (`installSummoningCircles`, `installDestructibleTowers`, `installWorkshop`, `applyRuinCells`, `registerActionIntercept`, `setSendPathOverride` + counterparts) as documented no-ops per ADR-0002 — they're preserved in case a future campaign decomposes cleanly into them, and the WorldMutator infra layer stays narrow-API-shaped even though no caller exercises that surface yet. `WorldMutator.test.ts` updated with a `makeHost()` helper that returns a complete vi.fn() host; the obsolete "no-op when host doesn't implement the method" case is deleted.

- **Item 6 — `GreenwardCampaignLifecycle` reachability wrapper (worked example for ADR-0003).** ADR-0003 grandfathers Greenward / Mech / Arcane on the legacy "controller stored as a `_*Controller` private field on GameScene" pattern indefinitely, but campaign #5 needs a worked example showing the migration path when a legacy campaign eventually needs DOM-side controller access. This commit adds the **minimum viable** migration: a thin `LifecycleAspect` (`GreenwardCampaignLifecycle`) whose `update`/`shutdown` are no-ops and whose role is purely to hold references to the controllers GameScene constructs in `installGreenwardRules`. DOM consumers reach the controller via `getActiveGreenwardController()` — a one-liner over `MissionRunner.getActiveLifecycle(GreenwardCampaignLifecycle)` — same pattern as `getActiveSnakeEyesController`. The 7 in-scene callsites that touch `_greenwardController` are unchanged; future migrations can move them through the wrapper one at a time. New unit test `GreenwardCampaignLifecycle.test.ts` pins null-before-set, populated-after-set, shutdown-nulls-refs, and accessor-returns-null-for-foreign-lifecycle semantics.

- **Item 7 — `MissionResult.custom` keys namespaced by `factionId`.** `MissionResult.custom` is a flat `Record<string, unknown>` consumed by every campaign's star objective predicates + UI surface. A naive key like `ruinsClaimed` is silently aliased if two campaigns happen to pick the same name — one campaign's predicate would read another's value, accepting a star it shouldn't (or rejecting one it should). The convention added to CONTEXT.md "Author conventions": new keys MUST be prefixed with the owning campaign's `factionId` and an underscore (e.g. `arcane_channelsInterrupted`, `void_mirrorWagerWon`). Existing un-prefixed keys (23 of them, originated by arcane/greenward/mech/voidc campaigns) are grandfathered via an explicit allowlist in the new pin test `src/data/campaigns/customKeys.test.ts`. The test extracts every `*.custom.X`, `*.custom['X']`, and `setCustom('X', ...)` reference across data/campaigns + greenward + voidc + GameScene; any new un-prefixed key fails the test with a message explaining how to fix. Sanity-tested by injecting a fake `r.custom.brandNewKey` reference, observing the test fail with the expected guidance, then reverting.

All 1345 tests pass (was 1338; +7 from `GreenwardCampaignLifecycle.test.ts` + the new customKeys pin). tsc clean. After items 1-7, the campaign #5 author can: copy Snake Eyes' typed accessor (item 1), populate `ui.endingPanel` (item 2), put per-mission state in a Mission Controller wrapper without ADR-0002 ceremony (items 3+6), pass typed `MissionEntry.campaign` payloads instead of new data-passthrough fields (item 4), trust tsc to catch missing host methods (item 5), and avoid silent collisions with existing campaigns by namespacing custom keys (item 7).
