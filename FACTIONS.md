# Factions

## Arcane (7 towers)
*"The arcane arts reward precision." — High fantasy magic.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Bolt | 25g | Reliable magic DPS. 4 upgrade levels. |
| Frost | 35g | 65% slow for 2.5s. No upgrades. |
| Storm | 55g | AoE splash. Good vs packs. |
| Focus | 90g | Long range sniper. 25% crit for 3x damage. |
| Mana Drain | 120g | Strips boss shields and damage-cap shields. |
| Meteor | 200g | Slow-falling projectile, massive AoE on impact. Location-targeted. |
| **Arcane Nova** | **700g** | Ultimate. AoE + crit + slow + damage amp combined. |

**Frontier:** Leyline Nexus (overcharge for burst, dormancy risk).

---

## Mechanical (8 towers)
*"Efficiency is the highest form of beauty." — Steampunk engineering.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Wall | 10g | Cheapest maze filler. No attack. |
| Turret | 30g | Ramps fire rate on same target. 4 levels. |
| Flamethrower | 40g | Short range AoE + burn DoT (8 DPS/3s). |
| Tesla | 80g | Chain lightning, jumps to 3+ targets. |
| Mortar | 120g | Extreme range artillery, huge splash. |
| Shredder | 150g | Very fast. Shreds armor tier for 4s. |
| Railgun | 300g | Pierces ALL creeps in a line. |
| **Titan Cannon** | **800g** | Ultimate. 500 damage, range 12, splash + burn + shred. |

**Frontier:** Deep Mine (dig deeper for +income, cave-in risk).

---

## Nature (6 towers)
*"All things grow in time." — Living forest, poison, synergy.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Thorn | 20g | Cheap physical DPS. 5 upgrade levels — scales all game. |
| Root | 35g | Strongest slow: 70% for 3s. |
| Blossom | 60g | No attack. Buffs adjacent towers: +15% DMG, +8% SPD/level. |
| Spore | 100g | Poisons ALL creeps near tower (not target). 2% HP/s. |
| Vine | 160g | 20% chance to root (stun) creeps for 0.8s. |
| **Elder Treant** | **600g** | Ultimate. Grows +8% damage permanently. Roots + buffs adjacents. |

**Frontier:** Sacred Grove (grows income, harvest for burst payout).

---

## Void (5 towers)
*"Embrace the chaos." — Gambling, gold generation, teleportation.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Gambler | 15g | 8% instant kill, 25% zero damage. Cheapest tower. |
| Spike | 30g | Each shot deals 50-150% damage randomly. |
| Siphon | 50g | +1g per hit. Economy engine. 4 levels. |
| Rift | 120g | Teleports creeps backward on path. |
| **Oblivion** | **900g** | Ultimate. 15% instakill, +3g/hit, extreme variance + damage amp. |

**Frontier:** The Rift (random 0-15g or 0-30g per wave).

---

## Military (6 towers)
*"Boots on the ground." — Mobile units, tactical control.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Sandbag | 8g | Cheapest wall. No attack. |
| Barbed Wire | 25g | Slows adjacent creeps 40%. No attack. |
| Rifleman | 40g | Mobile ranged unit. Moves to engage. |
| Brawler | 55g | Mobile melee. High damage up close. |
| Tank | 120g | Mobile AoE. Slow but long range, fires explosive shells. |
| **Commander** | **750g** | Ultimate. Mobile fighter + adjacency buff aura. |

**Frontier:** Supply Depot / Forward Base (steady income).

---

## Spawn Aliens (7 towers)
*"The hive hungers. Overwhelm them." — Cheap, fast, swarm.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Spitter | 12g | Extremely fast fire (250ms), 3 damage. |
| Stinger | 25g | Fast fire + weak poison (1% HP/s). |
| Swarm Node | 60g | No attack. All Alien towers in range fire 20% faster. |
| Acid Sprayer | 100g | Short AoE + armor shred + poison. |
| Hive Spire | 180g | Very fast chain, hits 5 targets. |
| Swarmling | 15g | Mobile melee. Cheap, fast, stackable. |
| **Overmind** | **700g** | Ultimate. Extreme fire rate + 30% faction speed aura. |

**Frontier:** Breeding Pool / Hive Queen (growth + harvest).

---

## Cypherpunk (7 towers)
*"Everything is data. Data can be rewritten." — Digital warfare.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Ping | 15g | Very long range (8+ tiles), low damage. |
| Firewall | 35g | Links to another Firewall. Beam damages creeps crossing between. |
| Virus | 55g | Infects target with DoT that spreads to nearby creeps. |
| Backdoor | 90g | Hacks creep — walks backward for 1.5s. |
| DDoS | 150g | AoE root (stun) all creeps in range for 0.5s. |
| Rootkit | 300g | Mutes mage abilities + heavy armor shred (2 tiers, 6s). |
| **Zero Day** | **800g** | Ultimate. Hack + virus + mute + shred in massive range. |

**Frontier:** Crypto Mine (gamble 0-25g) / Data Broker (overcharge).

---

## Infernal (6 towers)
*"Power at any price. Every flame has a cost." — Sacrifice and decay.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Imp | 10g | Decent damage. Expires after 4 waves. Disposable. |
| Hellfire | 45g | Strong AoE + burn. Loses 15% damage each wave. |
| Soul Drain | 70g | +2g per kill within range. Economy tower. |
| Fiend | 20g | KAMIKAZE. Runs to nearest creep, explodes for 80 AoE, destroys self. |
| Immolate | 200g | Strong DPS + burn. Can sacrifice for burst AoE. |
| **Apocalypse** | **900g** | Ultimate. Massive burn AoE + damage amp. |

**Frontier:** Soul Well (gamble 0-30g) / Blood Pact (dig, 15% collapse).

---

## Celestial (5 towers)
*"The light does not falter. The light endures." — Holy protection.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Acolyte | 25g | Light damage. 5% chance on nearby kill → +1 life. |
| Ward | 40g | No attack. Mutes all creep mage abilities in range. |
| Smite | 80g | High damage. +50% vs bosses and shielded creeps. |
| Sanctuary | 150g | Absorbs 1 leaked creep (recharges every 10 waves). |
| **Absolution** | **600g** | Ultimate. Holy AoE + 10% life gain on kill + mutes mages. |

**Frontier:** Tithe (steady) / Miracle (growth + harvest).

---

## Psionic (5 towers)
*"Your thoughts are not your own." — True damage, mind control.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Probe | 20g | True damage — ignores all armor completely. |
| Mesmer | 45g | Confuses target — walks backward for 1.2s. |
| Terror | 80g | Fear aura: 50% slow field + true damage. |
| Mind Spike | 150g | Long range true damage. +50% vs mage creeps. |
| **Overmind** | **750g** | Ultimate. Mass confusion + massive true damage pulse. |

**Frontier:** Dream Tap (gamble) / Mind Prison (overcharge).

---

## Harmonic (7 towers)
*"Alone, a whisper. Together, a symphony." — Aura network.*

| Tower | Cost | Key Traits |
|-------|------|------------|
| Resonator | 20g | Basic DPS. Weak alone, powerful with aura stacking. 4 levels. |
| Amplifier | 30g | No attack. +20% damage aura (red). Stacks with other Amplifiers. |
| Quickener | 40g | No attack. +15% fire rate aura (green). |
| Reach | 50g | No attack. +1.5 tile range aura (blue). |
| Critical Mass | 80g | No attack. 15% crit chance aura (magenta). |
| Conduit | 100g | Manual link: press L to link 2-3 aura towers. Shares their effects at 70%. |
| **Crescendo** | **650g** | Ultimate. Moderate DPS designed for maximum aura stacking. |

**How Conduit works:** Click Conduit → press L → click aura towers to link. Only links different aura types. Linked towers share their effects with each other at 70% strength. Click a linked tower to unlink. Max 2 links (3 at level 3).

**Re-emit:** Aura towers that receive inherited buffs from a conduit link will re-emit those buffs to their own neighbors at 50% of the received value (35% of original). Example: link a Quickener to an Amplifier — the Amplifier gains attack speed AND re-emits that speed buff to towers around it.

**Identity:** The placement puzzle faction. Individual towers are weak — the power comes from overlapping aura coverage. Two Amplifiers + Quickener on a Resonator = massive DPS. Conduit extends the network across the map, and re-emit means linked towers amplify each other's reach.

**Frontier:** Resonance Chamber (growth) / Symphony Hall (overcharge).

---

## Random (dynamic)
*"Adapt or die." — 6 random towers + 2 frontier buildings each wave.*

Each wave, 6 non-ultimate towers are randomly chosen from all faction pools. Bought towers stay permanently. Frontier buildings also rotate. Forces adaptive strategy.
